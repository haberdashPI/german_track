...
% verify that everythign works with CV
...
for i = 1:length(eeg_files)
    ...
    count = sum(cellfun(@(x) strcmp(x,'object'),stim_events{:,'condition'}));
    folds = k_folds(K,count);
    for k = 1:K
        test = folds{k,1};
        train = folds{k,2};

        % TODO: this isn't going to work, the indices for test and trian
        % aren't the same as the indices of the entire % set of objects

        male_model = trf_train(sprintf('trf_object_male_sid_%03d',i),...
            use_eeg,stim_info,lags,train,...
            @(i)strcmp(stim_events{i,'condition'},'object'),...
            @(i)load_sentence(stim_events,stim_info,i,male_index));

        fem1_model = trf_train(sprintf('trf_object_fem1_sid_%03d',i),...
            use_eeg,stim_info,lags,...
            @(i)strcmp(stim_events{i,'condition'},'object'),...
            @(i)load_sentence(stim_events,stim_info,i,fem1_index));
...
if use_fake_data
    writetable(table(male_C,fem1_C,fem2_C,other_male_C,sid),
        fullfile(cache_dir,'fake_testobj.csv'));
else
    writetable(table(male_C,fem1_C,fem2_C,other_male_C,sid),
        fullfile(cache_dir,'testobj.csv'));
end
alert()
