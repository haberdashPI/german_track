...
df = read.csv(file.path(cache_dir,"test_target.csv"))
...
ggsave(file.path(dir,"target_corr_vs_control.pdf"),width=9,height=7)
...
ggplot(dfcor,aes(x=pmax(fem1,fem2),y=male)) +
    ...
    scale_color_brewer(palette='Set1')
ggsave(file.path(dir,"target_corr_male_v_female_by_condition.pdf"),
    width=9,height=5)
...
ggplot(dfcor,aes(x=pmax(fem1,fem2),y=male)) +
    ...
    scale_color_brewer(palette='Set1')
ggsave(file.path(dir,"target_corr_male_v_female.pdf"),width=9,height=2.5)
