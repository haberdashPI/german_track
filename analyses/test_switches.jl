...
for eeg_file in eeg_files
    ...
    for cond in unique(stim_events.condition)
        ...
        male_model = trf_train(@sprintf("trf_%s_male_only_switch_sid_%03d",cond,sid),
            eeg,stim_info,lags,indices,
            name = @sprintf("Training SID %02d (Male): ",sid),
            bounds = bounds,
            i -> load_sentence(stim_events,stim_info,i,male_index))

        fem1_model = trf_train(@sprintf("trf_%s_fem1_only_switch_sid_%03d",cond,sid),
            eeg,stim_info,lags,indices,
            name = @sprintf("Training SID %02d (Female 1): ",sid),
            bounds = bounds,
            i -> load_sentence(stim_events,stim_info,i,fem1_index))

        fem2_model = trf_train(@sprintf("trf_%s_fem2_only_switch_sid_%03d",cond,sid),
            eeg,stim_info,lags,indices,
        ...
        C = trf_corr_cv(@sprintf("trf_%s_male_only_switch_sid_%s03d",cond,sid),eeg,
        ...
        C = trf_corr_cv(@sprintf("trf_%s_fem1_only_switch_sid%03d",cond,sid),eeg,
        ...
        C = trf_corr_cv(@sprintf("trf_%s_fem2_only_switch_sid%03d",cond,sid),eeg,
        ...
        C = trf_corr_cv(@sprintf("trf_%s_male_only_switch_sid%03d",cond,sid),eeg,
                stim_info,male_model,lags,indices,
...
save(joinpath(cache_dir,"test_only_switches.csv"),df)
