...
using EEGCoding, GermanTrack, DataFrames, Statistics, DataStructures,
    ...
    JSON3, JSONTables, Tables, Infiltrator, FileIO, BlackBoxOptim, RCall
...
lambdas = 10.0 .^ range(-2, 0, length=10)
resultdf = mapreduce(append!!, classcomps) do (comp, data)
    bywindow = pairs(groupby(data, [:winstart, :winlen]))
    ...
    foldxt(append!!, Map(findclass), collect(bywindow))
end

means = @_ resultdf |>
    groupby(__,[:winlen, :winstart, :comparison, :λ, :sid]) |>
    combine(__, [:correct, :weight] => wmeanish => :mean) |>
    groupby(__,[:winlen, :winstart, :comparison, :λ]) |>
    combine(__,:mean => mean => :mean)
    transform!(__, :winstart => ByRow(x -> fmt("1.2f",x)) => :winstart) |>
    transform!(__, :winlen   => ByRow(x -> fmt("1.2f",x)) => :winlen) |>
    rename!(__,:λ => :lambda)

# plot all data
R"""
ggplot($means, aes(x = factor(winlen), y = factor(winstart))) +
    geom_raster(aes(fill = mean)) +
    facet_grid(lambda~comparison)
"""

bestmeans = @_ resultdf |>
    groupby(__,[:winlen, :winstart, :comparison, :λ, :sid]) |>
    combine(__, [:correct, :weight] => wmeanish => :mean) |>
    groupby(__, [:comparison, :λ, :sid]) |>
    combine(__ , :mean => maximum => :mean) |>
    rename!(__,:λ => :lambda)

R"""
pl = ggplot($bestmeans, aes(x = lambda, y = mean, color = comparison)) +
    scale_x_log10() +
    ...
    stat_summary(geom = 'ribbon', fun.data = 'mean_cl_boot', alpha = 0.3,
        color = NA, aes(fill = comparison),
        fun.args = list(conf.int = 0.682))
"""

R"""
ggsave($(joinpath(dir, "lambdas_by_condition.pdf")), pl, width = 8, height = 6)
"""

# across, winlen, winstart, condition, fold
# find best winlen/winstart and average across conditions and fold
# report for each λ
