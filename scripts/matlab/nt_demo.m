...
if ~exist([processed_datadir, 'eeg', fname]);
    error('download data from http://www.fil.ion.ucl.ac.uk/spm/download/data/mmfaces/multimodal_eeg.zip');
end

% read data
h=ft_read_header([processed_datadir, 'eeg', fname]);
x=ft_read_data([processed_datadir, 'eeg', fname]);
x=x';
trig=x(:,144);
x=x(:,1:128);
sr=h.Fs;

% [p,x]=nt_read_data([processed_datadir, 'eeg', fname]);
% trig=x(:,144);
% x=x(:,1:128);
% sr=p.sr;
