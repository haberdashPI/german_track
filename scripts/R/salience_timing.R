...
model1 = aov(correct_mean ~ salience_label * winstart_label +
    Error(sid/(salience_label/winstart_label)), data = filter(df, condition=='object'))
summary(model1)

model1 = aov(correct_mean ~ salience_label * winstart_label +
    Error(sid/(salience_label/winstart_label)), data = filter(df, condition=='spatial'))
summary(model1)
...
hitmodel5 = stan_gamm4(correct_mean ~ s(winstart, by = salience_label),
    random = ~(1 | sid), data = hits, family = mgcv::betar)
