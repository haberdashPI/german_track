function [N_entire,N_single,P_Out] = loudness_1991(x, Pref, Fs, Mod)
...
%%%%%% Calculate main loudness without combining low frequencies %%%% NVM,
%%%%%% I don't have info on DDF and DCB (mostly DCB)  I'll just do raw P
%%%%%% and give it a try  Maybe come back to this with more info on the
%%%%%% processing

% Really could also just do with 
% A0_Mod = [zeros(1,8) A0];
% P_Out = P;
% for i = 1:length(P);
%     Le(i) = P(i);
...
% figure(1); clf; plot(ns); hold on; plot((1:length(Nm))*12,Nm,'r*');
