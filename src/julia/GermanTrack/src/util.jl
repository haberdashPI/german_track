...
export clear_cache!, plottrial, only_near, not_near, bound, subdict, padmeanpower, far_from,
    sample_from_ranges, ishit, find_decoder_training_trials
...
function single(x,message="Expected a single element.")
    @warn "Deprected in favor of `only`"
    only(x)
end
